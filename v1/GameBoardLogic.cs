...
public class GameBoardLogic
{
    ...
    //private TicTacToeGameState _inMemorySavedGameState;
    ...
    private void _TileClicked(BoardTilePosition position)
    {
        ...
        _gameData.Gamestate.CurrentPlayer = _PlayerTypeByIndex(_gameData.Gamestate.CurrentPlayerIndex);
    ...
    public PlayerType _PlayerTypeByIndex(int currentPlayer)
    {
        if (currentPlayer == 0)
        {
    ...
    private void DrawSignsFromBoard()
    {
        for (int i = 0; i < _gameData.Board.GetLength(0); i++)
        {
            for (int j = 0; j < _gameData.Board.GetLength(1); j++)
            {
                ...
                PlayerType player = _PlayerTypeByIndex(_gameData.Board[i, j]);
